...
| 5 | Detection (geofence, visit state machine, offline drift queue) | `features/detection`, `features/offline` | In progress | codex/detection | Plan: [detection-port-plan.md](detection-port-plan.md). Slices 1-2 (rules, county polygons, local store + repository) done. Needs a real-device test |
