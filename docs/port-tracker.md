...
| 5 | Detection (geofence, visit state machine, offline drift queue) | `features/detection`, `features/offline` | In progress | codex/detection | Plan: [detection-port-plan.md](detection-port-plan.md). Slices 1-3 (rules, county polygons, local store, sync queue) done; slice 4 (native geofencing) coded, needs a device test. Needs a real-device test |
